...
class TileExtractor:
    ...
    def extract_tiles(self, pil_img: Image.Image, grid_info: Dict, 
                     squares_per_tile: int = 12) -> List[TileInfo]:
        """
        ...
        if nx_total < squares_per_tile or ny_total < squares_per_tile:
            print(f"  Warning: Grid is {nx_total}x{ny_total}, too small for {squares_per_tile}x{squares_per_tile} tiles")
            return []
        ...
        for row in range(tiles_y):
            for col in range(tiles_x):
                ...
                # Extract the tile
                tile_crop = pil_img.crop((start_pixel_x, start_pixel_y, end_pixel_x, end_pixel_y))
                
                # Resize to target size
                tile_resized = tile_crop.resize((self.tile_size, self.tile_size), Image.Resampling.LANCZOS)
    ...
    def extract_tiles_with_overlap(self, pil_img: Image.Image, grid_info: Dict,
                                 squares_per_tile: int = 12, overlap_squares: int = 2) -> List[TileInfo]:
        """
