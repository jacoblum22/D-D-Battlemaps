...
class SmartImageSelector:
    ...
    def extract_dimensions_from_filename(self, filename: str) -> Optional[Tuple[int, int]]:
    ...
    def is_gridless(self, filename: str) -> bool:
        ...
        return any(re.search(pattern, name, re.IGNORECASE) for pattern in self.gridless_patterns)
    
    def is_gridded(self, filename: str) -> bool:
        """Check if filename indicates a gridded version"""
        name = Path(filename).stem.lower()
        return any(re.search(pattern, name, re.IGNORECASE) for pattern in self.gridded_patterns)
    ...
    def group_image_variants(self, image_list: List[Dict]) -> Dict[str, List[ImageVariant]]:
        """
    ...
    def select_best_variant(self, variants: List[ImageVariant]) -> ImageVariant:
        """
        Select the best variant according to the rules:
        1. If one filename is a substring of another, prefer the longer one
        2. If any variant has dimensions → prefer gridless
        3. If no variant has dimensions → prefer gridded  
        4. Fall back to any available version
        """
        if not variants:
        ...
        # Rule 1: Prefer longer filename when one is subsequence of another
        # Check all pairs to see if one filename is a subsequence of another
        ...
        # Rule 2: Check if any variant has dimensions
        has_dimensions = any(v.has_dimensions for v in variants)
        
        if has_dimensions:
            # Rule 2a: Prefer gridless when dimensions are available
            ...
            # If no explicit gridless, prefer the one with dimensions
            with_dims = [v for v in variants if v.has_dimensions]
            if with_dims:
                return with_dims[0]
        
        else:
            # Rule 3: Prefer gridded when no dimensions available
    ...
    def _get_selection_reason(self, variants: List[ImageVariant], selected: ImageVariant) -> str:
        ...
        if has_dimensions:
            if selected.is_gridless:
                return "gridless_with_dimensions"
            elif selected.has_dimensions:
                return "has_dimensions"
        else:
            if selected.is_gridded:
                return "gridded_no_dimensions"
...
def test_smart_selector():
    ...
    test_images = [
        ...
        {'path': '/maps/Forest_20x25_Gridless.png', 'filename': 'Forest_20x25_Gridless.png'},
    ...
    for img in test_images:
        ...
        print(f"  {filename}")
        print(f"    → Base: '{base_name}' | Gridless: {variant.is_gridless} | Gridded: {variant.is_gridded} | Dims: {variant.dimensions}")
    ...
    for base_name, variants in groups.items():
        print(f"\n� Group '{base_name}':")
        for v in variants:
            print(f"  - {v.filename} (gridless={v.is_gridless}, gridded={v.is_gridded}, dims={v.dimensions})")
    ...
    for img in selected:
        ...
        print(f"   Variants: {img['total_variants']} ({', '.join(img['variant_types'])})")
        if img['dimensions']:
