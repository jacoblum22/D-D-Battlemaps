...
class GridDetector:
    ...
    def detect_grid(self, pil_img: Image.Image) -> Optional[Dict]:
        ...
        for cell_size in cell_sizes:
            ...
            if width_error <= self.edge_tolerance_px and height_error <= self.edge_tolerance_px:
        ...
        for cell_size, nx, ny, w_err, h_err in edge_candidates:
            ...
            scored_candidates.append((final_score, score, cell_size, nx, ny, w_err, h_err))
    ...
    def _score_grid_candidate(self, v_map: np.ndarray, h_map: np.ndarray, 
                            W: int, H: int, nx: int, ny: int) -> float:
    ...
    def _alignment_score(self, intensity_map: np.ndarray, size: int, n_cells: int, direction: str) -> float:
    ...
    def _contrast_score(self, intensity_map: np.ndarray, size: int, n_cells: int, direction: str) -> float:
