...
class ImageProcessor:
    ...
    def save_tiles(
        self, tiles: List[TileInfo], source_name: str, output_dir: str
    ) -> int:
        """
    ...
    def save_tiles_organized(
        self,
        tiles: List[TileInfo],
        source_name: str,
        output_dir: str,
        organize_by: str = "source",
    ) -> int:
        """
    ...
    def create_summary_report(self, results: Dict[str, Any], output_dir: str) -> None:
        ...
        try:
            with open(report_path, "w") as f:
                ...
                f.write(
                    f"Total images processed: {results.get('images_processed', 0)}\n"
                )
                f.write(
                    f"Images with grids detected: {results.get('grids_detected', 0)}\n"
                )
                ...
                if "image_details" in results:
                    ...
                    for img_name, details in results["image_details"].items():
                        f.write(f"\n{img_name}:\n")
                        f.write(
                            f"  Grid detected: {details.get('grid_detected', False)}\n"
                        )
                        if details.get("grid_detected"):
                            f.write(
                                f"  Grid size: {details.get('grid_nx')}x{details.get('grid_ny')}\n"
                            )
                            f.write(
                                f"  Tiles extracted: {details.get('tiles_extracted', 0)}\n"
                            )
