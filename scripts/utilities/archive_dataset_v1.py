...
def archive_dataset():
    ...
    dataset_size_mb = sum(
        f.stat().st_size for f in Path("dataset_v1").rglob("*") if f.is_file()
    ) / (1024 * 1024)
    ...
    print(f"\nFiles in archive:")
    print(
        f"  dataset_v1/images/ ({manifest['dataset_summary'].get('successful', 0)} PNG files)"
    )
    print(
        f"  dataset_v1/captions/ ({manifest['dataset_summary'].get('successful', 0)} TXT files)"
    )
